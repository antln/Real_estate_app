...
def get_data_file():
    ...
    a = os.path.join(base_folder, 'data',
...
def load_file(filename):
    with open(filename, 'r', encoding='utf-8') as fin:
        ...
        for row in reader:
            # print(type(row), row)
            # print("Bed counts: {}".format(row['beds']))
...
def query_data(data):
    ...
    av_price = statistics.mean(prices)

    print("The average home price is ${:,}".format(int(av_price)))

    #prices1 = []
    #for pur in data:
    #    if pur.beds == 2:
    #        prices1.append(pur.price)
